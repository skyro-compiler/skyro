...
const SHIFT = 2 ** 8  # mjg can we eliminate this duplication?
...
func uint_add{range_check_ptr}(a : felt, b : felt) -> (res : felt):
    alloc_locals
    local res : felt
    local carry : felt
...
func uint_sub{range_check_ptr}(a : felt, b : felt) -> (res : felt):
    alloc_locals
    local res : felt
    local borrow : felt
    %{
...
# Multiplies two uint.
# Returns the result
func uint_mul{range_check_ptr}(a : felt, b : felt) -> (res : felt):
	alloc_locals
    local res : felt
    local offset : felt
...
func uint_unsigned_div{range_check_ptr}(a : felt, b : felt) -> (quotient : felt):
	alloc_locals
	local res : felt
    local rem : felt
...
func uint_not{range_check_ptr}(a : felt) -> (res : felt):
    return ((SHIFT - 1) - a)
end
...
func uint_neg{range_check_ptr}(a : felt) -> (res : felt):
...
func uint_eq{range_check_ptr}(a : felt, b : felt) -> (res):
    if a != b:
        return (0)
    else:
        return (1)
...
func uint_lt{range_check_ptr}(a : felt, b : felt) -> (res):
    tempvar res
...
func uint_le{range_check_ptr}(a : felt, b : felt) -> (res):
    tempvar res
	%{ids.res = 1 if ids.a <= ids.b else 0 %}
	# b < 2**128 & a < 2**128 is given by semantics
	# b < a
...
func uint_shl{range_check_ptr}(a : felt, b : felt) -> (res : felt):
...
#          But in cairo these are horrendibly slow
#		   Inconviniently these are the only one requiring imports and they are the only one that are not ap stable
func uint_shr{range_check_ptr}(a : felt, b : felt) -> (res : felt):
